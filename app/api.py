...
@api_application.route("/calc/log_base10/<op_1>", methods=["GET"])
def log_base10(op_1):
    try:
        num_1 = util.convert_to_number(op_1)
        return ("{}".format(CALCULATOR.log_base10(num_1)), http.client.OK, HEADERS)
    except ValueError  as e:
        return (str(e), http.client.BAD_REQUEST, HEADERS)

@api_application.route("/calc/raiz/<op_1>", methods=["GET"])
def raiz(op_1):
    try:
        num_1 = util.convert_to_number(op_1)
        return (str(CALCULATOR.raiz(num_1)), http.client.OK, HEADERS)
    except ValueError  as e:
        return (str(e), http.client.BAD_REQUEST, HEADERS)
